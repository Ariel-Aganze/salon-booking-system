...
const BookingsTable = ({ bookings, loading, error, onViewDetails, onUpdateStatus, onReschedule, onCancel }) => {
  ...
  const getPaymentStatus = (status) => {
    ...
    if (status === 'partial') return { label: 'Partial', color: 'bg-blue-100 text-blue-800' }
    if (status === 'pending') return { label: 'Pending', color: 'bg-orange-100 text-orange-800' }
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full">
          <thead className="bg-gray-50">
            <tr>
              {['Code', 'Customer', 'Service', 'Date', 'Time', 'Status', 'Payment', 'Amount', 'Actions'].map((header) => (
              ...
              return (
                <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-900">
                    {booking.reservation_code || booking.code}
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900">{booking.full_name || booking.customer_name}</div>
                ...
                {[...Array(totalPages).keys()].map((page) => (
                  <button
                    ...
                    className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                      currentPage === page + 1
                        ? 'z-10 bg-luxury-plum border-luxury-plum text-white'
